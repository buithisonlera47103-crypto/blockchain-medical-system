...
async function testBackendAPI() {
    ...
    try {
        ...
        const registerSuccess = registerResponse.statusCode === 200 && registerResponse.data?.userId;
        addTestResult(
            '用户注册功能',
            registerSuccess,
...
async function testDatabase() {
    ...
    try {
        ...
        // 从健康检查响应中获取数据库状态
        const dbStatus = response.data?.database || response.data?.services?.database;
        const isConnected = dbStatus === 'connected' || dbStatus === 'healthy';
...
async function testCache() {
    ...
    try {
        ...
        // 从健康检查响应中获取缓存状态
        const cacheStatus = response.data?.cache || response.data?.services?.cache;
        const isWorking = cacheStatus === 'connected' || cacheStatus === 'healthy';
...
async function testBlockchain() {
    ...
    try {
        ...
        // 从健康检查响应中获取区块链状态
        const blockchainStatus = response.data?.blockchain || response.data?.services?.blockchain;
        const isConnected = blockchainStatus === 'connected' || blockchainStatus === 'healthy';
