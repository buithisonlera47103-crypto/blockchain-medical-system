...
async function checkIPFSHealth(): Promise<ServiceStatus> {
  ...
  try {
    const ipfsService = await getService('ipfsService') as IPFSService;
...
async function checkBlockchainHealth(): Promise<ServiceStatus> {
  ...
  try {
    const blockchainService = await getService('blockchainService') as BlockchainService;
    ...
    return {
      status,
      responseTime,
      error: !connectionStatus.isConnected ? 'Blockchain connection failed' : undefined,
    };
  } catch (error) {
    const responseTime = Date.now() - startTime;
    return {
