...
describe('MedicalRecordService', () => {
  ...
  describe('Patient Records', () => {
    ...
    it('should return empty array when no records found', async () => {
      ...
      const records = await service.getUserRecords('patient-456', 'requester-123');
  ...
  describe('Health Check', () => {
    it('should perform health check', async () => {
      ...
      mockConnection.execute.mockResolvedValue([[{ count: 1 }]]);
      const records = await service.getUserRecords('patient-456', 'requester-123', 1, 10);
      const health = { status: 'healthy', recordCount: records.length };
  ...
  describe('Service Resource Management', () => {
    ...
    it('should handle database connection errors gracefully', async () => {
      ...
      await expect(service.getUserRecords('patient-456', 'requester-123', 1, 10)).rejects.toThrow();
    });
  });
});
