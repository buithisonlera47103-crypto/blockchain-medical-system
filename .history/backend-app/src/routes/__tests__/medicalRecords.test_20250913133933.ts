...
// Middleware imports for mocking
...
jest.mock('../../middleware/auth', () => ({
  ...
  optionalAuth: jest.fn((_req: any, _res: any, next: any) => {
    _req.user = {
      id: 'user-123',
      role: 'doctor'
    };
...
jest.mock('../../middleware/abac', () => jest.fn(() => (_req: any, _res: any, next: any) => next()));
...
describe('Medical Records Routes', () => {
  ...
  describe('DELETE /api/records/:id/share/:granteeId', () => {
    it('should revoke record access successfully', async () => {
      mockMedicalRecordService.revokeAccess.mockResolvedValue({
        success: true,
