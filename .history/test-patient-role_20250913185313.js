...
async function testPatientRole() {
  ...
  try {
    ...
    const registerResponse = await axios.post(`${BACKEND_URL}/api/auth/register`, patientData);
    ...
    const recordResponse = await axios.post(`${BACKEND_URL}/api/records`, recordData, {
      headers: { Authorization: `Bearer ${patientToken}` }
    });
    ...
    // 5. 查看自己的医疗记录
    console.log('\n📖 查看医疗记录...');
    const myRecordsResponse = await axios.get(`${BACKEND_URL}/api/records/my`, {
      headers: { Authorization: `Bearer ${patientToken}` }
    });
    ...
    try {
      const uploadResponse = await axios.post(`${BACKEND_URL}/api/files/upload`, fileData, {
        headers: { Authorization: `Bearer ${patientToken}` }
      });
      console.log('✅ 文件上传成功');
    } catch (error) {
