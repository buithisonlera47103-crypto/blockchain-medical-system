...
build system). However, depending on the OS, you make need to update the parsing
in mksysnum.
...
just include the error numbers and error strings, but also the signal numbers an
a wide variety of miscellaneous constants. The constants come from the list of
include files in the `includes_${uname}` variable. A regex then picks out the
